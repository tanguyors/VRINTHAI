      ...
      >
        <div className="container mx-auto px-6 flex items-center justify-between">
          <Link href="/" className="group flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#ffc857] to-[#ff8c42] group-hover:from-[#ff8c42] group-hover:to-[#ffc857] flex items-center justify-center shadow-lg transform group-hover:rotate-12 transition-transform duration-300">
                ...
                <Link
                  ...
                  className={`flex items-center gap-1 text-sm font-bold uppercase tracking-widest transition-colors ${
                    activeDropdown === link.name || isScrolled
                      ...
                      : "text-white/80 hover:text-[#ffc857]"
                  }`}
                >
                  {link.name}
                  {link.dropdown && (
                ...
                <AnimatePresence>
                  {link.dropdown && activeDropdown === link.name && (
                    <motion.div
                      ...
                      className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-[600px] bg-[#1e293b] border border-white/10 rounded-3xl p-6 shadow-[0_30px_100px_rgba(0,0,0,0.5)] backdrop-blur-2xl grid grid-cols-2 gap-2"
                    >
                      {link.dropdown.map((item) => (
                        ...
                        >
                          <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-[#cbd5e1] group-hover:bg-[#ffc857] group-hover:text-[#ffc857] transition-colors duration-300">
                            <item.icon className="w-5 h-5" />
                          </div>
                          <span className="text-white/90 font-medium group-hover:text-[#ffc857] transition-colors">
                            {item.name}
                          </span>
                        </Link>
                      ))}
          ...
          <div className="hidden lg:flex items-center gap-6">
            <div className="flex flex-col items-end">
              ...
              <span className="text-white font-bold text-sm flex items-center gap-2">
                <Phone className="w-3 h-3 text-[#cbd5e1] group-hover:text-[#ffc857] transition-colors duration-300" /> +66 61 420 2619
              </span>
            </div>
            <a href="https://wa.me/66614202619" target="_blank" rel="noopener noreferrer" className="group relative px-6 py-3 bg-gradient-to-r from-[#ffc857] to-[#ff8c42] text-white hover:from-[#ff8c42] hover:to-[#ffc857] hover:shadow-[0_25px_60px_rgba(255,200,87,0.4)] font-black text-sm rounded-xl transition-all duration-300 transform hover:scale-105 active:scale-95 shadow-[0_10px_30px_rgba(255,200,87,0.5)] flex items-center gap-2 overflow-hidden">
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[60] bg-[#0f172a] flex flex-col"
          >
            <div className="p-6 flex items-center justify-between border-b border-white/10">
              <div className="flex items-center gap-3">
                <Compass className="w-8 h-8 text-[#ffc857] transition-colors duration-300" />
                  ...
                  {link.dropdown && (
                    <div className="grid grid-cols-1 gap-3 pl-4 border-l-2 border-white/10">
                      {link.dropdown.map((sub) => (
                        ...
                        >
                          <sub.icon className="w-4 h-4 text-[#cbd5e1] group-hover:text-[#ffc857] transition-colors duration-300" />
                          {sub.name}
                        </Link>
                      ))}
                    </div>
            ...
            <div className="p-8 bg-[#1e293b] border-t border-white/10">
              <div className="mb-6 flex items-center gap-4">
                <div className="w-12 h-12 rounded-full border-2 border-[#cbd5e1] flex items-center justify-center">
                  <Phone className="w-5 h-5 text-[#cbd5e1] group-hover:text-[#ffc857] transition-colors duration-300" />
                </div>
                <div>
              ...
              <button className="w-full py-5 bg-gradient-to-r from-[#ffc857] to-[#ff8c42] text-white font-black text-lg rounded-2xl shadow-[0_20px_50px_rgba(255,200,87,0.5)] flex items-center justify-center gap-3">
